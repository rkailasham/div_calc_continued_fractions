...
L = constructL(Q,normQ,N);
y_at_A=L(k)*L(k);
drv_anltc_A = derv_lsq_kmin1_q_k(L,Q,normQ,j,N);
chkA=2*L(k)*derv_l_kmin1_q_k(L,Q,normQ,j,N);
...
L = constructL(Q,normQ,N);
y_at_B=L(k)*L(k);
drv_anltc_B = derv_lsq_kmin1_q_k(L,Q,normQ,j,N);
...
L = constructL(Q,normQ,N);
y_at_C=L(k)*L(k);
drv_anltc_C = derv_lsq_kmin1_q_k(L,Q,normQ,j,N);
