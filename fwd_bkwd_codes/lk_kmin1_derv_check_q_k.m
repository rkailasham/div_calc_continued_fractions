...
e1=plot(xval,yval,'-r','DisplayName','$L_{k-1}L_{k}$');
...
e2=plot(xval,derv_complete,'-.k','DisplayName','$\partial \left[L_{k-1}L_{k}\right]/\partial Q_{j}^{(x)}$');
...
L = constructL(Q,normQ,N);
y_at_A=L(k-1)*L(k);
drv_anltc_A = derv_l_kmin1_l_k_q_k(L,Q,normQ,j,N);

term1=L(k)*derv_l_kmin1_q_k(L,Q,normQ,k,N);
term2=L(k-1)*derv_l_k_q_k(L,Q,normQ,j,N);
chkA=term1+term2;
...
L = constructL(Q,normQ,N);
y_at_B=L(k-1)*L(k);
drv_anltc_B = derv_l_kmin1_l_k_q_k(L,Q,normQ,j,N);
% 
...
L = constructL(Q,normQ,N);
y_at_C=L(k-1)*L(k);
drv_anltc_C = derv_l_kmin1_l_k_q_k(L,Q,normQ,j,N);
% 
