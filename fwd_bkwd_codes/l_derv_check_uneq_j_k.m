...
e1=plot(xval,yval,'-k','DisplayName','$L^2_k$');
...
e2=plot(xval,derv_complete,'-.m','DisplayName','$\partial L^2_k/\partial Q_{j}^{(x)}$');
e2.MarkerFaceColor='m';
e2.MarkerSize=12;
e2.LineWidth=2;
...
L = constructL(Q,normQ,N);
y_at_A=L(k)*L(k);
drv_anltc_A = derv_l_kmin1_q_k(L,Q,normQ,j,N);
...
L = constructL(Q,normQ,N);
y_at_B=L(k)*L(k);
drv_anltc_B = derv_l_kmin1_q_k(L,Q,normQ,j,N);
...
L = constructL(Q,normQ,N);
y_at_C=L(k)*L(k);
drv_anltc_C = derv_l_kmin1_q_k(L,Q,normQ,j,N);
...
dim = [0.55 0.3 0.3 0.3];
str = {['$N_{\mathrm{s}} = $' num2str(N)],['$k = $' num2str(k),',\,$j = $' num2str(j)]};
annotation('textbox',dim,'String',str,'FitBoxToText','on','Interpreter','latex','FontSize',20,'LineStyle','-');



[h,icons,plots,legend_text]=legend({},'Location','southeast','FontSize',24,'Interpreter','latex','Box','on');
