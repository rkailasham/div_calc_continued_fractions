function [drvA,drvB,drvC,drv_anltc_A,drv_anltc_B,drv_anltc_C] = i_k_derv_anltc_numeric(chdim,N,Q,j,k,L,varphi,del,nlim)
...
xs='Q_{j}^{(%s)}';
specstring='\\partial I_k/\\partial Q_{j}^{(%s)}';
...
e1=plot(xval,yval,'-','DisplayName','$I_k$');
...
normQ = construct_norm(Q,N);
L = constructL(Q,normQ,N);
I_k_at_A=fwd_coeff_poly(k,L,p,N);
[drv_anltc_A] = derv_i_k_q_j(k,j,L,p,Q,normQ,N);
...
normQ = construct_norm(Q,N);
L = constructL(Q,normQ,N);
I_k_at_B=fwd_coeff_poly(k,L,p,N);
[drv_anltc_B] = derv_i_k_q_j(k,j,L,p,Q,normQ,N);
% 
...
normQ = construct_norm(Q,N);
L = constructL(Q,normQ,N);
I_k_at_C=fwd_coeff_poly(k,L,p,N);
[drv_anltc_C] = derv_i_k_q_j(k,j,L,p,Q,normQ,N);
% 
