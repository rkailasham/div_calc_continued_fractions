...
e1=plot(xval,yval,'-b','DisplayName','$L_k$');
...
e2=plot(xval,derv_complete,'-.r','DisplayName','$\partial L_k/\partial Q_{j}^{(x)}$');
...
L = constructL(Q,normQ,N);
y_at_A=L(k)*L(k);
drv_anltc_A = derv_l_k_q_k(L,Q,normQ,k,N);
...
L = constructL(Q,normQ,N);
y_at_B=L(k)*L(k);
drv_anltc_B = derv_l_k_q_k(L,Q,normQ,k,N);
...
L = constructL(Q,normQ,N);
y_at_C=L(k)*L(k);
drv_anltc_C = derv_l_k_q_k(L,Q,normQ,k,N);
